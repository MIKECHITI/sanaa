...
$prods = db()->query("
    SELECT p.id, p.price, p.artisan_id, p.stock, p.title
    FROM products p
...
// ── POST: Step 1 — save delivery details ─────────────────────
...
// ── POST: Step 2 — initiate M-Pesa STK Push ──────────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['initiate_mpesa'])) {
    ...
    // Validate phone before creating order
    if (!Mpesa::isValidPhone($mpesaPhone)) {
        $errors['mpesa'] = 'Invalid M-Pesa number. Use format 07XXXXXXXX.';
    } else {
        $user = currentUser();
        $pdo  = db();
        ...
        try {
            ...
            $pdo->prepare("
                INSERT INTO orders
                    (customer_id, order_number, subtotal, delivery_fee, total,
            ...
            ")->execute([
                ...
                $delivery['address'], $delivery['county'], $delivery['notes'],
            ]);
            $orderId = (int)$pdo->lastInsertId();

            // Insert order items + artisan earnings
            foreach ($items as $it) {
                $pdo->prepare("
                    INSERT INTO order_items
                        (order_id, product_id, artisan_id, quantity, unit_price, subtotal)
                    VALUES (?,?,?,?,?,?)
                ")->execute([
                    $orderId, $it['product_id'], $it['artisan_id'],
                ...
                $net     = $gross - $fee;
                $pdo->prepare("
                    INSERT INTO artisan_earnings
                        (artisan_id, order_item_id, gross_amount, platform_fee, net_amount)
                    VALUES (?,?,?,?,?)
            ...
            // Check for success: ResponseCode '0' = STK Push sent
            if (($mpesaResult['ResponseCode'] ?? null) === '0') {
                ...
                // STK Push failed — cancel the order
                $pdo->prepare("UPDATE orders SET status='cancelled', updated_at=NOW() WHERE id=?")
                    ->execute([$orderId]);

                $errors['mpesa'] = $mpesaResult['error']
                    ?? $mpesaResult['errorMessage']
                    ?? $mpesaResult['ResponseDescription']
                    ?? 'M-Pesa prompt failed. Check your number and try again.';
            }
    ...
    <div class="cart-layout">
        ...
        <div class="dash-card">
          ...
          <form method="POST">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="text" name="del_name"
                       class="input <?= isset($errors['del_name']) ? 'input-error' : '' ?>"
                       value="<?= e($_POST['del_name'] ?? $delivery_saved['name'] ?? '') ?>" required>
              </div>
              <div class="form-group">
                <label class="form-label">Phone Number *</label>
                <input type="tel" name="del_phone"
                       class="input <?= isset($errors['del_phone']) ? 'input-error' : '' ?>"
            ...
            <div class="form-group">
              ...
              <input type="text" name="del_address"
                     class="input <?= isset($errors['del_address']) ? 'input-error' : '' ?>"
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <select name="del_county" class="select">
                  <?php foreach (['Nairobi','Mombasa','Kisumu','Nakuru','Narok','Kisii','Kwale','Other'] as $c): ?>
                    <option value="<?= e($c) ?>" <?= ($delivery_saved['county'] ?? '') === $c ? 'selected' : '' ?>>
                      <?= e($c) ?>
                    </option>
                  <?php endforeach; ?>
                </select>
              ...
              <div class="form-group">
                ...
                <input type="text" name="del_notes" class="input"
      ...
      <div class="order-summary-card">
        ...
        <div class="summary-line total">
          <span>Total</span>
          <span style="color:var(--gold);font-family:'Playfair Display',serif;">
            <?= formatKES($total) ?>
          </span>
        </div>
